...
public sealed class TimeManager : NetworkBehaviour
{
    ...
    {
        if (NetworkManager.singleton != null && !string.IsNullOrWhiteSpace(lobbySceneName))
        {
            try { NetworkManager.singleton.ServerChangeScene(lobbySceneName); }
            catch (Exception e) { Debug.LogWarning($"[TimeManager] ServerChangeScene failed: {e.Message}"); }
        }
        else
        {
            Debug.LogWarning("[TimeManager] Lobby scene not set or NetworkManager missing.");
        }
    }
